...
_State diagram of the task workflow on this board. Tasks enter from the Backlog, move through In Progress and In Review, and end in Done, Blocked, or Won't Do. The per-column item counts live in the **Board Status** table below._

```mermaid
stateDiagram-v2
    accTitle: Task workflow states on the board
    accDescr: Six-state task workflow. Tasks enter at Backlog and end at Done, Blocked, or Won't Do. Blocked tasks return to Backlog when unblocked. In Review can bounce back to In Progress when changes are requested.

    [*] --> Backlog
    Backlog --> In_Progress : Pulled by assignee
    Backlog --> Wont_Do : Explicitly declined
...
> ⚠️ Always show all 6 column rows in the table below — Even if a column has no items, include it with a placeholder. This makes the board structure explicit and ensures categories are never forgotten. Use a placeholder like [No items yet] when a column is empty.
